...
function pList(){let h='';MILESTONES.forEach(ms=>{const items=tasks.filter(t=>t.ms===ms);if(!items.length)return;const done=items.filter(t=>t.st==='Done').length;const pct=Math.round(done/items.length*100);
  h+=`<div class="ms"><div class="ms-head"><span class="nm">${ms}</span><span class="ct">${done}/${items.length} done</span><span class="prog"><span class="derive">percent_done</span><span class="bar"><span class="fill" style="width:${pct}%"></span></span><span class="pc">${pct}%</span></span></div><div class="mlist">`;
...
function empList(){const depts=[...new Set(EMP.map(e=>e.dept))];return depts.map(d=>{const items=EMP.filter(e=>e.dept===d);const act=items.filter(e=>e.status==='Active').length;
  return `<div class="lg"><div class="lg-head"><span class="pill">${d==='—'?'No department':d}</span><span class="ct">${items.length}</span><span class="sum">${act} active</span></div><div class="llist">${items.map(e=>`<div class="lrow" onclick="hrOpenEmp('${e.code}')">${eav(e)}<span class="nm">${e.name}</span><span class="co">${e.role} · ${e.code}</span><span class="lstat"><span class="dot" style="background:${EST[e.status]||'var(--muted)'}"></span>${e.status}</span></div>`).join('')}</div></div>`;}).join('');}
...
function collList(c,rows){const gs=[...new Set(rows.map(r=>r.type))];return gs.map(g=>{const items=rows.filter(r=>r.type===g);const sum=items.reduce((a,r)=>a+(r.balance||0),0);
  return `<div class="lg"><div class="lg-head"><span class="pill">${g}</span><span class="ct">${items.length}</span><span class="sum">${fmt(sum)}</span></div><div class="llist">${items.map(r=>`<div class="lrow" onclick="collOpen('${r.id}')"><span class="eav" style="background:${r.color};width:26px;height:26px;font-size:var(--size-caption)">${r.av}</span><span class="nm">${r.name}</span><span class="co">${r.owner} · ${r.terms}</span><span class="val">${fmt(r.balance)}</span></div>`).join('')}</div></div>`;}).join('');}
