...
function empList(){const depts=[...new Set(EMP.map(e=>e.dept))];return depts.map(d=>{const items=EMP.filter(e=>e.dept===d);return `<div class="lg"><div class="lg-head"><span class="pill">${d==='—'?'No department':d}</span><span class="ct">${items.length}</span></div>${items.map(e=>`<div class="lrow" onclick="hrOpenEmp('${e.code}')">${eav(e)}<span class="nm">${e.name}</span><span class="co">${e.role} · ${e.code}</span><span style="color:${EST[e.status]};font-weight:var(--weight-semibold);font-size:var(--size-caption)">${e.status}</span></div>`).join('')}</div>`;}).join('');}
...
function empHead(e){return `<div class="lead-head"><div class="lh-id"><h1>${e.name}</h1>
    <div class="byline">${eav(e,26)} <b>${e.role}</b> <span class="dotsep">·</span> <span style="color:${EST[e.status]||'var(--muted)'};font-weight:var(--weight-semibold)">${e.status}</span>${e.dept!=='—'?` <span class="dotsep">·</span> ${e.dept} team`:''}${e.mgr!=='—'?` <span class="dotsep">·</span> reports to ${e.mgr}`:''} <span class="dotsep">·</span> joined ${e.join}</div></div></div>`;}
...
function collHead(r){return `<div class="lead-head"><div class="lh-id"><h1>${r.name}</h1>
    <div class="byline"><span class="eav" style="background:${r.color};width:26px;height:26px;font-size:var(--size-caption)">${r.av}</span> <b>${r.type}</b> <span class="dotsep">·</span> <span style="color:${stColor(r.status)};font-weight:var(--weight-semibold)">${r.status}</span> <span class="dotsep">·</span> owner ${r.owner} <span class="dotsep">·</span> ${r.terms}</div></div></div>`;}
...
function vnHead(v){return `<div class="lead-head"><div class="lh-id"><h1>${v.name}</h1>
    <div class="byline"><span class="av" style="background:${v.color}">${v.av}</span> <b>${v.type}</b> <span class="dotsep">·</span> <span style="color:${stColor(v.status)};font-weight:var(--weight-semibold)">${v.status}</span> <span class="dotsep">·</span> owner ${v.owner}</div></div></div>`;}
