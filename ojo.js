...
function av(id){if(!id)return '<div class="av empty">?</div>';const p=PEOPLE[id];return `<div class="av" style="--ac:${p[1]}" title="${p[2]}">${p[0]}</div>`;}
...
function projPanelCells(){const a=projScore();const done=tasks.filter(t=>t.st==='Done').length;const total=tasks.length||1;
  return [
   ...
   {render:'facts',props:{rows:[['Timeline',`${a[0]}% complete`],['Due','20 Jul 2026'],['Budget','₹3,80,000 left'],['Tasks',`${done}/${total} done`],['Owner',`<span class="ip-owner"><span class="av" style="--ac:#F04D56">PN</span>Priya Nair</span>`]]}},
...
function mountDash(){
  document.getElementById('modcontent').innerHTML=`<div class="box">
    ...
    <div class="pgrid">${PROJECTS.map(p=>`<div class="pcard" onclick="${p.open?"go('project')":"toast('Demo: only Apollo is wired')"}">
      ...
      <div class="avs">${p.team.map(t=>`<div class="av" style="--ac:${t[1]}">${t[0]}</div>`).join('')}</div></div>`).join('')}
...
function wMessages(){return PMSGS.map(m=>`<div class="d-msg" onclick="toast('Open: ${m[3]}')"><div class="av2" style="--ac:${m[1]}">${m[0]}</div><div style="min-width:0"><div class="mt">${m[3]}</div><div class="mb">${m[4]}</div><div class="who">${m[2]}${pcommMini(m[2])}</div></div></div>`).join('');}
function wTodos(){return ['Design','Build'].map(ms=>{const its=tasks.filter(t=>t.ms===ms);const done=its.filter(t=>t.st==='Done').length;return `<div class="d-todo-h"><span class="pie"></span>${ms} <span style="color:var(--faint);font-weight:var(--weight-semibold)">${done}/${its.length}</span></div>`+its.map(t=>`<div class="d-titem ${t.st==='Done'?'done':''}"><button class="ck" onclick="event.stopPropagation();toggleDone('${t.id}')"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><path d="M20 6 9 17l-5-5"/></svg></button><span class="tx" onclick="openPeek('${t.id}')">${t.t}</span></div>`).join('');}).join('');}
...
function wChat(){return PCHATS.map(c=>`<div class="d-chat"><div class="av2" style="--ac:${c[1]}">${c[0]}</div><div><div class="cw">${c[2]}</div><div class="cm">${c[3]}</div></div></div>`).join('');}
...
function contactsListHTML(list,addLabel){const add=addLabel===null?'':`<button class="contact-add" onclick="toast('${addLabel||'Add a contact'}')">${svg('<path d="M12 5v14M5 12h14"/>',13)} ${addLabel||'Add a contact'}</button>`;
  return `<div class="contacts-list">${list.map(c2=>`<div class="contact-row"><span class="av" style="--ac:${c2[3]}">${(c2[0]||'?').split(' ').map(w=>w[0]).slice(0,2).join('')}</span>
...
function cmtHTML(c,i){return `<div class="cmt"><span class="av" style="--ac:${c.color}">${c.av}</span>
...
function commentsHTML(noun){const cs=rec.comments;
  return `<div class="cmts">
   ...
   <div class="cmt-new"><span class="av" style="--ac:var(--ink-bg)">VV</span>
...
function leadHead(l){const own=l.asg?PEOPLE[l.asg]:null;const st=LSTAGES.find(s=>s.k===l.st)||{};
  return `<div class="lead-head"><div class="lh-id"><h1>${l.co}</h1>
    <div class="byline lh-meta"><span class="lh-stage"><span class="dot" style="background:${st.cc||'var(--ink)'}"></span>${l.st}</span><span class="lh-sep">·</span><span class="lh-val">${fmt(l.val)}</span><span class="lh-sep">·</span>${own?`<span class="ip-owner"><span class="av" style="--ac:${own[1]}">${own[0]}</span>${own[2]}</span>`:`<button class="lh-assign" onclick="toast('Assign owner')">${svg('<path d="M12 5v14M5 12h14"/>',11)} Assign owner</button>`}</div>
  </div></div>`;}
function taskHead(t){const p=PEOPLE[t.asg];const st=STATUSES.find(s=>s.k===t.st)||{};
  return `<div class="lead-head"><div class="lh-id"><h1>${t.t}</h1>
    <div class="byline lh-meta"><span class="lh-stage"><span class="dot" style="background:${st.cc||'var(--ink)'}"></span>${t.st}</span><span class="lh-sep">·</span><span>${t.pri} priority</span><span class="lh-sep">·</span><span>${t.due?'Due '+t.due:'No due date'}</span><span class="lh-sep">·</span><span class="ip-owner"><span class="av" style="--ac:${p[1]}">${p[0]}</span>${p[2]}</span></div>
  </div></div>`;}
function leadRecHTML(){const l=rec.ent;
  return `
...
function taskRecHTML(){const t=rec.ent;if(!t.accept)t.accept=[];if(!t.proof)t.proof=[];
  ...
  <div class="trk ${trkRunning?'on':''}" id="trkCard">
    ...
    <div class="mid"><div class="plan">${t.est} <span class="lbl" style="display:inline;font-weight:var(--weight-semibold)">planned</span></div><div class="track"><div id="trkBar" style="width:${pct}%"></div></div><div class="lbl" id="trkPct">${pct}% planned time elapsed</div></div>
  ...
  <div class="tsec acc" id="accSec"><div class="tsec-h acc-head" onclick="toggleAcc()">Acceptance &amp; Review <span class="chev">${svg(I.chev,16)}</span></div>
    <div class="acc-body">
      ...
      ${t.proof.length?`<div>${t.proof.map(p=>`<span class="proof-chip">${svg(p.type==='link'?I.link:I.file,14)} ${p.name}</span>`).join('')}</div>`:`<div class="dropz">No files or links added yet. Upload a file or add a link to get started.<br><span style="font-size:var(--size-caption)">Any file type — max 50MB per file</span></div>`}
...
function renderDocs(l){if(!l.docs)l.docs=makeDocs(l);const d=l.docs;
  return `<div class="docpipe">${d.pipe.map(p=>`<div class="docchip ${p.k===d.cur?'on':''}" onclick="docSelect('${p.k}')"><div class="dci">${svg(DOCICON[p.k],18)}</div><div style="flex:1"><div class="dcn">${p.label}</div><div class="dcs">${p.sub}</div></div><div class="dcr" style="color:${p.pct===100?'var(--ok)':'var(--warn)'}">${p.pct}%</div></div>`).join('')}</div>
   ...
   ${d.open?docBar(d):''}`;}
function docBlocksHTML(d){const arr=d.blocks[d.cur];return arr.map((b,i)=>docBlockHTML(b,i)).join('')+`<div class="dadd" onclick="docAddOpen(event,${arr.length})">${svg('<path d="M12 5v14M5 12h14"/>',15)} Add a block, or press <b style="font-weight:var(--weight-semibold);margin:0 2px">/</b> for blocks</div>`;}
...
function flySize(px){const f=document.getElementById('flyout');if(!f)return;f.style.flexBasis=px+'px';f.style.width=px+'px';
  ...
  /* keep #tbOjo spanning the panel width even when collapsed, so the Ojo Genie pill
     stays at the SAME position whether the panel is open or closed (no jump) */
  if(o&&shellMode!=='merged'){const w=(px>0?px:flyW)+'px';o.style.width=w;o.style.flexBasis=w;}
...
function genieSel(f){
  ...
  if(section==='genie'&&fly&&fly.classList.contains('show')&&b&&b.querySelector('.gh-band')){
    b.querySelectorAll('.gha').forEach(x=>x.classList.toggle('on',x.dataset.face===f));
...
function empTable(){return `<div class="tablewrap"><table><thead><tr><th>Employee Code</th><th>Employee Name</th><th>Role</th><th>Department</th><th>Manager</th><th>Created on</th><th>Status</th></tr></thead><tbody>
   ${EMP.map(e=>`<tr onclick="hrOpenEmp('${e.code}')"><td>${e.code}</td><td><span class="owncell">${eav(e)}<b style="color:var(--navy)">${e.name}</b></span></td><td>${e.role}</td><td>${e.dept}</td><td>${e.mgr}</td><td class="co">${e.created}</td><td><span style="color:${EST[e.status]||'var(--muted)'};font-weight:var(--weight-semibold)">${e.status}</span></td></tr>`).join('')}</tbody></table></div>`;}
...
function empList(){const depts=[...new Set(EMP.map(e=>e.dept))];return depts.map(d=>{const items=EMP.filter(e=>e.dept===d);const act=items.filter(e=>e.status==='Active').length;
  return `<div class="lg"><div class="lg-head"><span class="pill"><span class="dot" style="background:var(--line-3)"></span>${d==='—'?'No department':d}</span><span class="ct">${items.length}</span><span class="sum">${act} active</span></div><div class="llist">${items.map(e=>`<div class="lrow" onclick="hrOpenEmp('${e.code}')">${eav(e)}<span class="nm">${e.name}</span><span class="co">${e.role} · ${e.code}</span><span class="lstat"><span class="dot" style="background:${EST[e.status]||'var(--muted)'}"></span>${e.status}</span></div>`).join('')}</div></div>`;}).join('');}
...
function empHead(e){return `<div class="lead-head"><div class="lh-id"><h1>${e.name}</h1>
    <div class="byline lh-meta"><span class="lh-stage"><span class="dot" style="background:${EST[e.status]||'var(--muted)'}"></span>${e.status}</span><span class="lh-sep">·</span><span>${e.role}</span>${e.dept!=='—'?`<span class="lh-sep">·</span><span>${e.dept}</span>`:''}${e.mgr!=='—'?`<span class="lh-sep">·</span><span>Reports to ${e.mgr}</span>`:''}<span class="lh-sep">·</span><span>Joined ${e.join}</span></div></div></div>`;}
...
function hrLeaves(){const reqs=[['REL0003','Casual leave','12–13 Jun 2026','Pending'],['REL0002','Sick leave','05 Jun 2026','Approved'],['REL0004','Work from home','03 Jun 2026','Approved']];
  ...
  return `<div class="sec" style="margin-top:24px"><div class="sec-h">Leave requests</div><div class="tablewrap"><table><thead><tr><th>Employee</th><th>Type</th><th>Dates</th><th>Status</th><th></th></tr></thead><tbody>${reqs.map(r=>{const e=EMP.find(x=>x.code===r[0]);return `<tr><td><span class="owncell">${eav(e)}<b style="color:var(--navy)">${e.name}</b></span></td><td>${r[1]}</td><td class="co">${r[2]}</td><td><span class="badge"><span class="dot" style="background:${sc[r[3]]}"></span>${r[3]}</span></td><td>${r[3]==='Pending'?`<button class="vlink" onclick="toast('Leave approved')">Approve</button>`:''}</td></tr>`;}).join('')}</tbody></table></div></div>`;}
function hrHolidays(){const hols=[['Independence Day','15 Aug 2026','Fri'],['Onam','26 Aug 2026','Wed'],['Gandhi Jayanti','02 Oct 2026','Fri'],['Diwali','08 Nov 2026','Sun']];
  return `<div class="sec" style="margin-top:24px"><div class="sec-h">Upcoming holidays</div><div class="llist">${hols.map(h=>`<div class="lrow"><span class="nm">${h[0]}</span><span class="co">${h[2]}</span><span class="val" style="color:var(--ink)">${h[1]}</span></div>`).join('')}</div></div>`;}
function hrAttSet(t){hrAttTab=t;renderHRPage();}
function hrAttV(v){hrAttView=v;renderHRPage();}
function hrAttList(){
  const att=[['REL0001','Present','09:02','18:14','9h 12m'],['REL0003','Present','09:20','17:55','8h 35m'],['REL0004','On leave','—','—','—'],['REL0002','Present','09:48','18:30','8h 42m'],['REL0006','Absent','—','—','—']];
  const sc={Present:'var(--ok)',Absent:'var(--warn-ink)','On leave':'var(--info)'};
...
function profAttendance(){const days=[['02 Jun 2026','Present','09:58 AM','06:31 PM','8h 33m'],['03 Jun 2026','Present','10:04 AM','06:45 PM','8h 41m'],['04 Jun 2026','Present','09:46 AM','06:20 PM','8h 34m'],['05 Jun 2026','Absent','-','-','-']];
  return `${pageHeader('Attendance','Your check-ins, hours & leave balance','<rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/>')}
   ...
   <div class="tablewrap"><table><thead><tr><th>Date</th><th>Status</th><th>Check In</th><th>Check Out</th><th>Work Hours</th></tr></thead><tbody>${days.map(d=>`<tr><td class="co">${d[0]}</td><td><span style="color:${d[1]==='Present'?'var(--ok)':'var(--coral-ink)'};font-weight:var(--weight-semibold)">${d[1]}</span></td><td>${d[2]}</td><td>${d[3]}</td><td>${d[4]}</td></tr>`).join('')}</tbody></table></div>`;}
...
function adminUsers(){return `<div class="hrtoolbar"><div class="hsearch">${svg(SVS.search,15)} Search users</div><button class="mtool" style="margin-left:auto">${svg(SVS.filter,17)}</button><button class="mtool">${svg(SVS.sort,17)}</button></div>
   <div class="tablewrap"><table><thead><tr><th style="width:40px"><input type="checkbox" onclick="event.stopPropagation()"></th><th>Name</th><th>Role</th><th>Department</th><th>Manager</th><th>Joining Date</th><th style="width:44px"></th></tr></thead><tbody>${ORGUSERS.map(u=>`<tr onclick="toast('Open user: ${u[2]}')"><td><input type="checkbox" onclick="event.stopPropagation()"></td><td><span class="owncell"><span class="eav" style="--ac:${u[1]};width:30px;height:30px;font-size:var(--size-caption)">${u[0]}</span><span style="color:var(--navy);font-weight:var(--weight-bold);font-size:var(--size-body)">${u[2]}</span></span></td><td>${roleChip(u[3])}</td><td class="co">${u[4]}</td><td class="co">${u[5]}</td><td class="co">${u[6]}</td><td><button class="iconedit" onclick="event.stopPropagation();toast('User actions')">${svg(SVS.more,16)}</button></td></tr>`).join('')}</tbody></table></div>
...
function adminRoles(){return `<div class="hrtoolbar"><div class="hr-h" style="margin:0">Roles &amp; Permissions</div><button class="abtn dark" style="margin-left:auto;padding:9px 16px;border-radius:999px;font-weight:var(--weight-semibold);font-size:var(--size-body)" onclick="toast('New role')">＋ New Role</button></div>
   <div class="tablewrap"><table><thead><tr><th>Role</th><th>Base Role</th><th>Members</th><th>Permissions</th><th style="width:44px"></th></tr></thead><tbody>${ORGROLES.map((r,i)=>`<tr onclick="adminOpenRole(${i})"><td><span class="owncell"><span class="rdot" style="background:${r.color}"></span><span style="color:var(--navy);font-weight:var(--weight-bold);font-size:var(--size-body)">${r.name}</span></span></td><td class="co">${r.base}</td><td class="co">${r.members} members</td><td><span class="ebadge" style="color:var(--navy);background:var(--surface-2)">${r.perm}</span></td><td><button class="iconedit" onclick="event.stopPropagation();adminOpenRole(${i})">${svg(SVS.pencil,15)}</button></td></tr>`).join('')}</tbody></table></div>`;}
...
function adminRoleEdit(r){const colors=['#C92F3A','#2F6FED','#7C53E6','#15A06A','#E08A1E','#E0533A','#0EA5A5','#6366F1'];
  ...
  return `<div class="roleedit-head"><button class="back" onclick="adminBackRoles()">${svg(SVS.arrow,18)}</button><div><div class="hr-h" style="margin:0">Edit Role</div><div class="muted2">Configure who can view, create, update or delete records across modules.</div></div></div>
   ...
   <div class="roleedit-foot"><button class="pill" onclick="adminBackRoles()">Cancel</button><button class="abtn dark" style="padding:9px 22px;border-radius:999px;font-weight:var(--weight-semibold);font-size:var(--size-body)" onclick="adminSaveRole()">Update</button></div>`;}
...
function metAddOpen(e,ctx){e.stopPropagation();const m=document.getElementById('metpal');const avail=Object.keys(METRIC_DEFS[ctx]||{}).filter(id=>!METR[ctx].includes(id));
  m.innerHTML='<div class="h">Add metric</div>'+(avail.length?avail.map(id=>`<button class="pi" onclick="metAdd('${ctx}','${id}')"><span class="ic">${svg(SVS.trend,15)}</span>${METRIC_DEFS[ctx][id][0]}</button>`).join(''):'<div style="padding:8px 10px;color:var(--ghost);font-size:var(--size-body)">All metrics added</div>');
...
function dashTable(){return `<div class="tablewrap"><table><thead><tr><th>Project</th><th>Client</th><th>Status</th><th class="num">Tasks</th><th>Due</th><th class="num">Progress</th></tr></thead><tbody>${PROJECTS.map(p=>`<tr onclick="${p.open?"go('project')":"toast('Demo: only Apollo is wired')"}"><td><span class="owncell"><span class="eav" style="--ac:${p.color};width:26px;height:26px;font-size:var(--size-caption)">${p.ic}</span><b style="color:var(--navy)">${p.name}</b></span></td><td class="co">${p.client}</td><td><span style="color:${p.sc};font-weight:var(--weight-semibold)">${p.status}</span></td><td class="num">${p.done}/${p.total}</td><td class="co">${p.due}</td><td class="num">${p.pct}%</td></tr>`).join('')}</tbody></table></div>`;}
function dashBoard(){const sts=[...new Set(PROJECTS.map(p=>p.status))];return `<div class="board color">`+sts.map(s=>{const items=PROJECTS.filter(p=>p.status===s);const cc=items[0]?items[0].sc:'var(--faint)';return `<div class="col"><div class="col-head" style="--cc:${cc}"><span class="pill"><span class="dot"></span>${s}</span><span class="ct">${items.length}</span></div><div class="col-body">${items.map(p=>`<div class="card" onclick="${p.open?"go('project')":"toast('Demo')"}"><div class="nm">${p.name}</div><div class="by">${p.client}</div><div class="mid"><span class="val">${p.pct}%</span></div><div class="foot"><span class="src">${p.done}/${p.total} tasks</span><span class="eav" style="--ac:${p.color};width:23px;height:23px;font-size:var(--size-caption);margin-left:auto">${p.ic}</span></div></div>`).join('')||'<div class="col-empty">None</div>'}</div></div>`;}).join('')+`</div>`;}
function dashList(){const sts=[...new Set(PROJECTS.map(p=>p.status))];return sts.map(s=>{const items=PROJECTS.filter(p=>p.status===s);const cc=items[0]?items[0].sc:'var(--faint)';const done=items.reduce((a,p)=>a+p.done,0),total=items.reduce((a,p)=>a+p.total,0);
  return `<div class="lg"><div class="lg-head"><span class="pill"><span class="dot" style="background:${cc}"></span>${s}</span><span class="ct">${items.length}</span><span class="sum">${done}/${total} tasks</span></div><div class="llist">${items.map(p=>`<div class="lrow" onclick="${p.open?"go('project')":"toast('Demo')"}"><span class="eav" style="--ac:${p.color};width:26px;height:26px;font-size:var(--size-caption)">${p.ic}</span><span class="nm">${p.name}</span><span class="co">${p.client} · ${p.done}/${p.total} tasks</span><span class="val">${p.pct}%</span></div>`).join('')}</div></div>`;}).join('');}
...
function collCell(r,col){const v=r[col[1]];if(col[2]==='money')return fmt(v);if(col[2]==='badge')return `<span style="color:${stColor(v)};font-weight:var(--weight-semibold)">${v}</span>`;if(col[2]==='b')return `<span class="owncell"><span class="eav" style="--ac:${r.color};width:26px;height:26px;font-size:var(--size-caption)">${r.av}</span><b style="color:var(--navy)">${v}</b></span>`;
  ...
  if(col[2]==='score'){const sc=vnScore(r);const t=scoreColors(sc.pct);return `<span style="color:${t[1]};font-weight:var(--weight-semibold)">${sc.pct}%</span> <span class="co" style="font-size:var(--size-caption)">${sc.label}</span>`;}
  return v;}
...
function collBoard(c,rows){return `<div class="board color">`+c.statuses.map(s=>{const items=rows.filter(r=>r.status===s[0]);return `<div class="col"><div class="col-head" style="--cc:${s[1]}"><span class="pill"><span class="dot"></span>${s[0]}</span><span class="ct">${items.length}</span></div><div class="col-body">${items.map(r=>`<div class="card" onclick="collOpen('${r.id}')"><div class="nm">${r.name}</div><div class="by">${r.type} · ${r.owner}</div><div class="foot"><span class="val">${fmt(r.balance)}</span><span class="eav" style="--ac:${r.color};width:23px;height:23px;font-size:var(--size-caption);margin-left:auto">${r.av}</span></div></div>`).join('')||'<div class="col-empty">None</div>'}</div></div>`;}).join('')+`</div>`;}
function collList(c,rows){const gs=[...new Set(rows.map(r=>r.type))];return gs.map(g=>{const items=rows.filter(r=>r.type===g);const sum=items.reduce((a,r)=>a+(r.balance||0),0);
  return `<div class="lg"><div class="lg-head"><span class="pill"><span class="dot" style="background:var(--line-3)"></span>${g}</span><span class="ct">${items.length}</span><span class="sum">${fmt(sum)}</span></div><div class="llist">${items.map(r=>`<div class="lrow" onclick="collOpen('${r.id}')"><span class="eav" style="--ac:${r.color};width:26px;height:26px;font-size:var(--size-caption)">${r.av}</span><span class="nm">${r.name}</span><span class="co">${r.owner} · ${r.terms}</span><span class="val">${fmt(r.balance)}</span></div>`).join('')}</div></div>`;}).join('');}
...
function collHead(r){return `<div class="lead-head"><div class="lh-id"><h1>${r.name}</h1>
    <div class="byline lh-meta"><span class="lh-stage"><span class="dot" style="background:${stColor(r.status)}"></span>${r.status}</span><span class="lh-sep">·</span><span>${r.type}</span><span class="lh-sep">·</span><span>Owner ${r.owner}</span><span class="lh-sep">·</span><span>${r.terms}</span></div></div></div>`;}
function collBody(c,r){
  ...
  return head+acctTopInsights(r)+`<div class="bgrid" style="margin-top:18px">
    ...
    <div class="bsec"><div class="bsec-h">Recent transactions</div><div class="bcard">
      ${acctTxns(r).map(t=>`<div class="wkv"><span class="k" style="color:var(--ink);font-weight:var(--weight-semibold)">${t.id}</span><span class="v">${fmt(t.amt)} <span style="margin-left:8px">${acctTxnPill(t.status)}</span></span></div>`).join('')}
...
function vnScorePill(s){if(s==null)return '<span class="co">—</span>';const t=scoreColors(s);return `<span style="color:${t[1]};font-weight:var(--weight-semibold)">${s}%</span>`;}
...
const VNW={
 ...
 projects:{title:'Sub-contracted projects',body:v=>(v.projects||[]).map(p=>`<div class="wkv"><span class="k" style="color:var(--ink);font-weight:var(--weight-semibold)">${p.name}</span><span class="v">${vnScorePill(p.score)} <span style="color:${VNPSTC[p.status]||'var(--muted)'};font-weight:var(--weight-semibold);font-size:var(--size-caption);margin-left:8px">${p.status}</span></span></div>`).join('')+`<div class="wkv"><span class="k">Average project score</span><span class="v">${vnScorePill(vnAvgScore(v))}</span></div><div class="wkv"><span class="k"></span><span class="v"><a style="color:var(--info);font-weight:var(--weight-semibold);cursor:pointer" onclick="vnSetTab('Projects')">View all →</a></span></div>`},
...
function vnBody(){const v=curVn();
  ...
  if(vnTab==='Projects')return head+((v.projects||[]).length
    ?`<div class="tablewrap" style="margin-top:6px"><table><thead><tr><th>Project</th><th class="num">Budget</th><th>Created on</th><th>Project score</th><th>Status</th><th>BRO</th></tr></thead><tbody>${v.projects.map(p=>`<tr onclick="toast('Open ${p.name} (demo)')"><td><b style="color:var(--navy)">${p.name}</b></td><td class="num">${fmt(p.bud)}</td><td class="co">${p.created}</td><td>${vnScorePill(p.score)}</td><td><span style="color:${VNPSTC[p.status]||'var(--muted)'};font-weight:var(--weight-semibold)">${p.status}</span></td><td><a class="vlink" onclick="event.stopPropagation();toast('Open ${p.bro} (demo)')">${p.bro} ↗</a></td></tr>`).join('')}</tbody></table></div>`
...
function vnVendCell(v){return `<span class="owncell"><span class="eav" style="--ac:${v.color};width:24px;height:24px;font-size:var(--size-caption)">${v.av}</span><b style="color:var(--navy)">${v.name}</b></span>`;}
...
function vnShortResults(){const ranked=vnShortRank(),need=vnReqSvcs;
  ...
  return rec+`<div class="plwrap">${ranked.map((r,i)=>{const t=scoreColors(r.fit);
    ...
    return `<div class="plrow ${i===0&&need.length?'top':''}" onclick="vnOpen('${r.v.id}')">
      ...
      <span class="eav" style="--ac:${r.v.color};width:34px;height:34px;font-size:var(--size-caption);flex:0 0 34px">${r.v.av}</span>
      <div class="plmeta"><div class="plname">${r.v.name}</div><div class="plsub">${r.v.type} · ${r.sc.onTime}% on-time · ${r.v.engage.join(' · ')}${r.inBudget===true?' · <span style="color:var(--ok);font-weight:var(--weight-semibold)">within budget</span>':r.inBudget===false?' · <span style="color:var(--warn);font-weight:var(--weight-semibold)">over budget</span>':''}</div><div class="plchips">${chips}</div></div>
      <div class="plfit"><div class="pf" style="color:${t[1]}">${r.fit}%</div><div class="pl">fit</div></div>
      <div class="plfit" style="min-width:54px"><div class="pf" style="font-size:var(--size-body);color:var(--navy)">${r.sc.pct}%</div><div class="pl">OJO</div></div>
...
function lsPricing(){
  ...
  return lsLegend(`Each <b>service</b> is a domain cell with a <code>rate model</code> and <code>price</code>. Grouped by category; bind these into packages or quote them straight on a proposal.`)
   ...
   ${SVC_CATALOG.map(g=>`<tr class="svcgrp"><td><b style="color:var(--navy)">${g.group}</b></td><td>${g.rate==='Group'?'<span class="co">Group</span>':`<span class="ratechip" style="color:${RATECLR[g.rate]}">${g.rate}</span>`}</td><td class="num">${g.price}</td><td></td></tr>
     ${g.items.map(it=>`<tr><td style="padding-left:30px">${it[0]}<div style="font-size:var(--size-caption);color:var(--faint)">Under ${g.group}</div></td><td><span class="ratechip" style="color:${RATECLR[it[1]]}">${it[1]}</span></td><td class="num">${it[2]}</td><td><button class="rowmenu" onclick="event.stopPropagation();lsOpen('service','${it[0].replace(/'/g,"")}')">${svg(SVS.pencil,14)}</button></td></tr>`).join('')}`).join('')}</tbody></table></div>`;}
...
function ctRenderList(){const rows=CONTACTS.filter(c=>ctFilter==='All'||c.type===ctFilter);
  document.getElementById('ctwork').innerHTML=`<div class="tablewrap"><table><thead><tr><th>Contact</th><th>Organisation</th><th>Email</th><th>Phone</th><th>Type</th></tr></thead><tbody>
   ${rows.map(c=>`<tr onclick="ctOpen('${c.id}')"><td><span class="owncell"><span class="eav" style="--ac:${c.color};width:26px;height:26px;font-size:var(--size-caption)">${c.av}</span><b style="color:var(--navy)">${c.name}</b></span></td><td>${ctOrgCell(c)}</td><td class="co">${c.email||'—'}</td><td class="co">${c.phone}</td><td>${ctBadge(c.type)}</td></tr>`).join('')}</tbody></table></div>`;}
...
function ctBody(){const c=curCt();
  const head=`<div class="lead-head"><div class="lh-id"><h1>${c.name}</h1>
    <div class="byline"><span class="eav" style="--ac:${c.color};width:26px;height:26px;font-size:var(--size-caption)">${c.av}</span> ${ctBadge(c.type)} <span class="dotsep">·</span> <b>${c.org}</b>${c.role?` <span class="dotsep">·</span> ${c.role}`:''} <span class="dotsep">·</span> added ${c.added}</div></div></div>`;
  ...
  return head+ctTopInsights(c)+`<div class="bgrid" style="margin-top:18px">
    <div class="bsec"><div class="bsec-h">Reach them</div><div class="bcard">
      ...
      <div class="wkv"><span class="k">Email</span><span class="v" style="font-weight:var(--weight-medium);font-size:var(--size-body)">${c.email||'—'}</span></div>
...
function orgBody(){const o=orgData(orgRec);
  const head=`<div class="lead-head"><div class="lh-id"><h1>${o.name}</h1>
    <div class="byline"><span class="eav" style="--ac:${o.color};width:26px;height:26px;font-size:var(--size-caption)">${o.av}</span> ${ctBadge(o.type)} <span class="dotsep">·</span> <span style="color:var(--ok);font-weight:var(--weight-semibold)">Active</span> <span class="dotsep">·</span> ${o.people.length} contact${o.people.length===1?'':'s'}${o.loc!=='—'?` <span class="dotsep">·</span> ${o.loc}`:''}</div></div></div>`;
  ...
  return head+`<div class="bgrid" style="margin-top:14px">
    <div class="bsec"><div class="bsec-h">Organisation</div><div class="bcard">
      ...
      <div class="wkv"><span class="k">Email</span><span class="v" style="font-weight:var(--weight-medium);font-size:var(--size-body)">${o.email}</span></div>
    ...
    <div class="bsec"><div class="bsec-h">Team</div><div class="bcard">
      ${o.people.map(c=>`<div class="wkv" style="cursor:pointer" onclick="ctOpen('${c.id}')"><span class="k" style="color:var(--ink);font-weight:var(--weight-semibold)"><span class="owncell"><span class="eav" style="--ac:${c.color};width:24px;height:24px;font-size:var(--size-caption)">${c.av}</span>${c.name}</span></span><span class="v">${ctBadge(c.type)}</span></div>`).join('')||orgEmpty('<circle cx="9" cy="8" r="3"/><path d="M3 20a6 6 0 0 1 12 0"/>','No people yet')}
    ...
    <div class="bsec"><div class="bsec-h">Deals</div><div class="bcard">
      ${o.deals.length?o.deals.map(l=>`<div class="wkv" style="cursor:pointer" onclick="openDetail('lead','${l.id}')"><span class="k" style="color:var(--ink);font-weight:var(--weight-semibold)">${l.nm} · ${l.st}</span><span class="v">${fmt(l.val)}</span></div>`).join(''):orgEmpty('<rect x="3" y="7" width="18" height="13" rx="2"/><path d="M8 7V5a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/>','No deals yet')}
    ...
    <div class="bsec"><div class="bsec-h">Projects</div><div class="bcard">
      ${o.projects.length?o.projects.map(p=>`<div class="wkv" style="cursor:pointer" onclick="vnOpenTab('${o.v.id}','Projects')"><span class="k" style="color:var(--ink);font-weight:var(--weight-semibold)">${p.name}</span><span class="v">${vnScorePill(p.score)}</span></div>`).join(''):orgEmpty('<rect x="3" y="4" width="18" height="16" rx="2"/><path d="M3 9h18M9 4v16"/>','No projects yet')}</div></div>
...
function acctAccounting(){const tab=acctTab||'journals';
  ...
  return head
   +`<div style="font-weight:var(--weight-semibold);color:var(--navy);margin:14px 0 10px">New Entry</div>`
...
function acctSettings(){const tab=acctTab||'fy';
  ...
  return head
   +`<div style="display:flex;align-items:flex-start;margin:6px 0 12px"><div><div style="font-weight:var(--weight-semibold);color:var(--navy);font-size:var(--size-subheading)">Fiscal Years ${svg('<circle cx="12" cy="12" r="9"/><path d="M12 11v5M12 8h.01"/>',13)}</div><div class="muted2">Manage your organization's financial years</div></div><div style="margin-left:auto"><button class="abtn dark" style="padding:9px 16px;border-radius:10px;font-weight:var(--weight-semibold);font-size:var(--size-body);display:inline-flex;align-items:center;gap:7px;background:var(--navy);color:#fff" onclick="toast('New Fiscal Year')">${svg(SVS.plus,14)} New Fiscal Year</button></div></div>`
   ...
   +`<div style="font-weight:var(--weight-semibold);color:var(--navy);font-size:var(--size-subheading);margin:18px 0 4px">GST Settings ${svg('<circle cx="12" cy="12" r="9"/><path d="M12 11v5M12 8h.01"/>',13)}</div><div class="muted2" style="margin-bottom:10px">Configure your organization's GST number</div><div style="border:1px solid var(--line);border-radius:14px;padding:16px 18px;display:flex;align-items:center"><div><div class="muted2">GSTIN</div><div style="font-weight:var(--weight-semibold);color:var(--navy)">${SELLER.gstin}</div></div><div style="margin-left:auto;display:flex;gap:10px"><button class="iconedit">${svg(SVS.pencil,15)}</button><button class="iconedit">${svg('<path d="M3 6h18M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/>',15)}</button></div></div>`;}
...
function acctJournal(j){const tot=j.entries.reduce((a,e)=>[a[0]+e[2],a[1]+e[3]],[0,0]);
  return `<div class="docpage">
   <h1 style="font-size:var(--size-heading);font-weight:var(--weight-semibold);color:var(--navy)">${j.narr}</h1>
   <div class="muted2" style="margin:2px 0 18px">${j.id} • System Generated</div>
   <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px"><div style="font-weight:var(--weight-semibold);font-size:var(--size-subheading);color:var(--navy)">${j.voucher}</div><span style="margin-left:auto">${apill(j.status)}</span></div>
   ...
   <div style="font-weight:var(--weight-semibold);color:var(--muted);margin-bottom:6px">Narration:</div><div style="margin-bottom:18px">${j.narr}</div>
   <div style="font-weight:var(--weight-semibold);color:var(--muted);margin-bottom:8px">Entries</div>
...
function acctInvoiceDoc(i){const s=SELLER;const nfx=n=>new Intl.NumberFormat('en-IN',{minimumFractionDigits:2,maximumFractionDigits:2}).format(n);const item=i.item||'Professional services',rate=i.rate||i.amount;
  return `<div class="docpage">
   <h1 style="font-size:var(--size-heading);font-weight:var(--weight-semibold);color:var(--navy);display:flex;align-items:center;gap:12px">#${i.id} <span class="ebadge" style="color:var(--ok);background:var(--ok-soft)">Tax Invoice</span> ${apill(i.status)}</h1>
   <div style="border:1px solid var(--line);border-radius:18px;padding:26px 30px;margin-top:18px;background:var(--surface)">
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:18px"><span class="eav" style="--ac:var(--navy);width:30px;height:30px;font-size:var(--size-caption)">R</span><div style="font-weight:var(--weight-semibold);font-size:var(--size-subheading);color:var(--navy)">${s.name}</div></div>
...
function acctBillDoc(b){const items=b.items||[['Item','Expense',1,b.amount||0,0]];const tds=b.tds||0,paid=b.paid||0,payable=(b.amount||0)-tds,bal=b.balance!=null?b.balance:payable-paid;
  return `<div class="docpage">
   <h1 style="font-size:var(--size-heading);font-weight:var(--weight-semibold);color:var(--navy);display:flex;align-items:center;gap:10px">#${b.id} ${apill(b.status)} ${b.paid?apill('Partially Paid'):''}</h1>
...
function acctPayrunDoc(p){return `<div class="docpage">
   <h1 style="font-size:var(--size-heading);font-weight:var(--weight-semibold);color:var(--navy);display:flex;align-items:center;gap:10px">${p.title} ${apill(p.status)}</h1>
   <div style="background:var(--surface-2);border:1px solid var(--line);border-radius:16px;padding:22px 24px;margin-top:16px;display:flex;flex-wrap:wrap;gap:34px;align-items:center">
    <div><div style="font-size:var(--size-heading);font-weight:var(--weight-bold);color:var(--navy)">${p.emps}</div><div class="muted2">Employees</div></div>
    <div><div style="font-size:var(--size-heading);font-weight:var(--weight-bold);color:var(--navy)">${fmt(p.cost)}</div><div class="muted2">Total Payroll Cost</div></div>
    <div><div style="font-size:var(--size-heading);font-weight:var(--weight-bold);color:var(--navy)">${fmt(p.tax)}</div><div class="muted2">Taxes &amp; deductions</div></div>
    <div><div style="font-size:var(--size-heading);font-weight:var(--weight-bold);color:var(--navy)">${fmt(p.net)}</div><div class="muted2">Net Payable</div></div>
...
function tBody(){const t=tRec,p=PEOPLE[t.asg];
  return `<h1>${t.title}</h1>
   <div class="byline"><span class="av" style="--ac:${p[1]}">${p[0]}</span> ${p[2]} · linked to <b>${t.link}</b></div>
   ${tTopInsights(t)}
   ${tStepper(t)}
   <div class="rows">
...
function homeDayStrip(){let days='';for(let i=0;i<7;i++){const d=homeWeekStart+i;const sel=d===homeSelDay,today=d===11;const n=TASKS.filter(t=>t.status!=='Done'&&homeDay(t)===d).length;
   days+=`<button class="ds-day ${sel?'on':''} ${today?'today':''}" onclick="homeSelectDay(${d})"><span class="ds-wd">${WD[wdOf(d)]}</span><span class="ds-n">${d}</span>${n?'<span class="ds-dot"></span>':''}</button>`;}
