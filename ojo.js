...
function taskHead(t){const p=PEOPLE[t.asg];const st=STATUSES.find(s=>s.k===t.st)||{};
  return `<div class="lead-head"><div class="lh-id"><h1>${t.t}</h1>
    <div class="byline lh-meta"><span class="lh-stage"><span class="dot" style="background:${st.cc||'var(--ink)'}"></span>${t.st}</span><span class="lh-sep">·</span><span>${t.pri} priority</span><span class="lh-sep">·</span><span>${t.due?'Due '+t.due:'No due date'}</span><span class="lh-sep">·</span><span class="ip-owner"><span class="av" style="background:${p[1]}">${p[0]}</span>${p[2]}</span></div>
  </div></div>`;}
...
function adminUsers(){return `<div class="hrtoolbar"><div class="hsearch">${svg(SVS.search,15)} Search users</div><button class="mtool" style="margin-left:auto">${svg(SVS.filter,17)}</button><button class="mtool">${svg(SVS.sort,17)}</button></div>
   <div class="tablewrap"><table><thead><tr><th style="width:40px"><input type="checkbox" onclick="event.stopPropagation()"></th><th>Name</th><th>Role</th><th>Department</th><th>Manager</th><th>Joining Date</th><th style="width:44px"></th></tr></thead><tbody>${ORGUSERS.map(u=>`<tr onclick="toast('Open user: ${u[2]}')"><td><input type="checkbox" onclick="event.stopPropagation()"></td><td><span class="owncell"><span class="eav" style="background:${u[1]};width:30px;height:30px;font-size:var(--size-caption)">${u[0]}</span><span style="color:var(--navy);font-weight:var(--weight-bold);font-size:var(--size-body)">${u[2]}</span></span></td><td>${roleChip(u[3])}</td><td class="co">${u[4]}</td><td class="co">${u[5]}</td><td class="co">${u[6]}</td><td><button class="iconedit" onclick="event.stopPropagation();toast('User actions')">${svg(SVS.more,16)}</button></td></tr>`).join('')}</tbody></table></div>
...
function dashList(){const sts=[...new Set(PROJECTS.map(p=>p.status))];return sts.map(s=>{const items=PROJECTS.filter(p=>p.status===s);const cc=items[0]?items[0].sc:'var(--faint)';const done=items.reduce((a,p)=>a+p.done,0),total=items.reduce((a,p)=>a+p.total,0);
  return `<div class="lg"><div class="lg-head"><span class="pill"><span class="dot" style="background:${cc}"></span>${s}</span><span class="ct">${items.length}</span><span class="sum">${done}/${total} tasks</span></div><div class="llist">${items.map(p=>`<div class="lrow" onclick="${p.open?"go('project')":"toast('Demo')"}"><span class="eav" style="background:${p.color};width:26px;height:26px;font-size:var(--size-caption)">${p.ic}</span><span class="nm">${p.name}</span><span class="co">${p.client} · ${p.done}/${p.total} tasks</span><span class="val">${p.pct}%</span></div>`).join('')}</div></div>`;}).join('');}
...
function collList(c,rows){const gs=[...new Set(rows.map(r=>r.type))];return gs.map(g=>{const items=rows.filter(r=>r.type===g);const sum=items.reduce((a,r)=>a+(r.balance||0),0);
  return `<div class="lg"><div class="lg-head"><span class="pill"><span class="dot" style="background:var(--line-3)"></span>${g}</span><span class="ct">${items.length}</span><span class="sum">${fmt(sum)}</span></div><div class="llist">${items.map(r=>`<div class="lrow" onclick="collOpen('${r.id}')"><span class="eav" style="background:${r.color};width:26px;height:26px;font-size:var(--size-caption)">${r.av}</span><span class="nm">${r.name}</span><span class="co">${r.owner} · ${r.terms}</span><span class="val">${fmt(r.balance)}</span></div>`).join('')}</div></div>`;}).join('');}
...
function vnVendCell(v){return `<span class="owncell"><span class="eav" style="background:${v.color};width:24px;height:24px;font-size:var(--size-caption)">${v.av}</span><b style="color:var(--navy)">${v.name}</b></span>`;}
...
function vnShortResults(){const ranked=vnShortRank(),need=vnReqSvcs;
  ...
  return rec+`<div class="plwrap">${ranked.map((r,i)=>{const t=scoreColors(r.fit);
    ...
    return `<div class="plrow ${i===0&&need.length?'top':''}" onclick="vnOpen('${r.v.id}')">
      ...
      <span class="eav" style="background:${r.v.color};width:34px;height:34px;font-size:var(--size-caption);flex:0 0 34px">${r.v.av}</span>
...
function ctRenderList(){const rows=CONTACTS.filter(c=>ctFilter==='All'||c.type===ctFilter);
  document.getElementById('ctwork').innerHTML=`<div class="tablewrap"><table><thead><tr><th>Contact</th><th>Organisation</th><th>Email</th><th>Phone</th><th>Type</th></tr></thead><tbody>
   ${rows.map(c=>`<tr onclick="ctOpen('${c.id}')"><td><span class="owncell"><span class="eav" style="background:${c.color};width:26px;height:26px;font-size:var(--size-caption)">${c.av}</span><b style="color:var(--navy)">${c.name}</b></span></td><td>${ctOrgCell(c)}</td><td class="co">${c.email||'—'}</td><td class="co">${c.phone}</td><td>${ctBadge(c.type)}</td></tr>`).join('')}</tbody></table></div>`;}
...
function ctBody(){const c=curCt();
  const head=`<div class="lead-head"><div class="lh-id"><h1>${c.name}</h1>
    <div class="byline"><span class="eav" style="background:${c.color};width:26px;height:26px;font-size:var(--size-caption)">${c.av}</span> ${ctBadge(c.type)} <span class="dotsep">·</span> <b>${c.org}</b>${c.role?` <span class="dotsep">·</span> ${c.role}`:''} <span class="dotsep">·</span> added ${c.added}</div></div></div>`;
...
function orgBody(){const o=orgData(orgRec);
  const head=`<div class="lead-head"><div class="lh-id"><h1>${o.name}</h1>
    <div class="byline"><span class="eav" style="background:${o.color};width:26px;height:26px;font-size:var(--size-caption)">${o.av}</span> ${ctBadge(o.type)} <span class="dotsep">·</span> <span style="color:var(--ok);font-weight:var(--weight-semibold)">Active</span> <span class="dotsep">·</span> ${o.people.length} contact${o.people.length===1?'':'s'}${o.loc!=='—'?` <span class="dotsep">·</span> ${o.loc}`:''}</div></div></div>`;
  ...
  return head+`<div class="bgrid" style="margin-top:14px">
    ...
    <div class="bsec"><div class="bsec-h">Team</div><div class="bcard">
      ${o.people.map(c=>`<div class="wkv" style="cursor:pointer" onclick="ctOpen('${c.id}')"><span class="k" style="color:var(--ink);font-weight:var(--weight-semibold)"><span class="owncell"><span class="eav" style="background:${c.color};width:24px;height:24px;font-size:var(--size-caption)">${c.av}</span>${c.name}</span></span><span class="v">${ctBadge(c.type)}</span></div>`).join('')||orgEmpty('<circle cx="9" cy="8" r="3"/><path d="M3 20a6 6 0 0 1 12 0"/>','No people yet')}
...
function acctInvoiceDoc(i){const s=SELLER;const nfx=n=>new Intl.NumberFormat('en-IN',{minimumFractionDigits:2,maximumFractionDigits:2}).format(n);const item=i.item||'Professional services',rate=i.rate||i.amount;
  return `<div class="docpage">
   ...
   <div style="border:1px solid var(--line);border-radius:18px;padding:26px 30px;margin-top:18px;background:var(--surface)">
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:18px"><span class="eav" style="background:var(--navy);width:30px;height:30px;font-size:var(--size-caption)">R</span><div style="font-weight:var(--weight-semibold);font-size:var(--size-subheading);color:var(--navy)">${s.name}</div></div>
